...
$(document).ready(function(){
    //Xử  lý chuyển hướng sang dang ký
    $('#signup-btn').click(function(event){
        event.preventDefault(); // Ngăn hành vi mặc định (nếu dùng thẻ <a>)
        window.location.href = "/signup"; // Chuyển hướng đến endpoint /signup
    })
    ...
    //Ajax là kỹ thuật cho phép web gửi/nhận dữ liệu từ sever mà ko cần tải lại trang
        $.ajax({
            url: '/login',
            method: 'POST',
    ...
    $('#signup-form').submit(function(event){
        ...
        $('#signup-error-msg').text('Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ in hoa, chữ thường, số và ký tự đặc biệt (!@#$%^&).').show();
        ...
        $.ajax({
            ...
            success:function(response){
                if (response.success) {
                    ...
                    $('signup-error-msg').text(response.message);
